  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter(
      (payment) =>
        ...
        payment.classId.toString().includes(keyword),
    );
  }, [payments, query]);
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-sm text-gray-500">Chưa có thanh toán phù hợp.</div>
        ) : (
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead className="bg-gray-50 text-left text-xs uppercase tracking-wide text-gray-500">
              <tr>
                ...
                <th className="px-5 py-3 text-right font-medium">Ngày ghi nhận</th>
              </tr>
            </thead>
              ...
              {filteredPayments.map((payment) => (
                <tr key={payment.id} className="hover:bg-gray-50">
                  <td className="px-5 py-4">
                    ...
                    <div className="mt-0.5 text-xs text-gray-500">{payment.studentPhoneNumber}</div>
                  </td>
