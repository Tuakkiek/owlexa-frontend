...
export const ScheduleRuleForm = ({ teachers, onSubmit, onCancel }: ScheduleRuleFormProps) => {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    if (startTime >= endTime) return setError("Giờ bắt đầu phải trước giờ kết thúc.");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="rounded-card border border-surface-border bg-surface-page p-4 text-sm text-gray-600">
        Quy tắc này chỉ cần ngày bắt đầu. Hệ thống sẽ lặp theo số buổi đã cấu hình trong khóa học, rồi tự tính ngày kết thúc.
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Giáo viên</span>
          <select value={teacherUserId} onChange={(e) => setTeacherUserId(e.target.value ? Number(e.target.value) : "")} className="w-full rounded-input border border-surface-border bg-white px-3 py-2 text-sm focus:border-primary focus:outline-none">
        ...
        <label className="block">
          ...
          <select value={roomId} onChange={(e) => setRoomId(e.target.value ? Number(e.target.value) : "")} className="w-full rounded-input border border-surface-border bg-white px-3 py-2 text-sm focus:border-primary focus:outline-none">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <label className="block">
          ...
          <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full rounded-input border border-surface-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Giờ bắt đầu</span>
          <input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full rounded-input border border-surface-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
        <label className="block">
          <span className="mb-1 block text-sm font-medium text-gray-700">Giờ kết thúc</span>
          <input type="time" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full rounded-input border border-surface-border px-3 py-2 text-sm focus:border-primary focus:outline-none" />
        </label>
      </div>

      <label className="block">
        <span className="mb-1 block text-sm font-medium text-gray-700">Loại lịch lặp</span>
        <select value={type} onChange={(e) => setType(e.target.value as "THEORY_CLASS" | "ONLINE_CLASS")} className="w-full rounded-input border border-surface-border bg-white px-3 py-2 text-sm focus:border-primary focus:outline-none">
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" isLoading={isSubmitting}>Tạo rule</Button>
      </div>
    </form>
  );
};
